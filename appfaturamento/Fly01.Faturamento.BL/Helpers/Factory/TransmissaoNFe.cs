    ...
    public abstract class TransmissaoNFe
    {
        ...
        public EmissaoNFe.Produto ObterProduto(NFeProduto item)
        {
            return new EmissaoNFe.Produto()
            {
                ...
                Quantidade = Math.Round(item.Quantidade, 2),
                QuantidadeTributada = Math.Round(item.Quantidade, 2),
                UnidadeMedida = item.Produto.UnidadeMedida?.Abreviacao,
                UnidadeMedidaTributada = item.Produto.UnidadeMedida?.Abreviacao,
                ValorBruto = Math.Round((item.Quantidade > 0 ? (Math.Round(item.Quantidade, 2) * Math.Round(item.Valor,2)) : Math.Round(item.Valor,2)), 2),
                ValorUnitario = Math.Round(item.Valor, 2),
                ValorUnitarioTributado = Math.Round(item.Valor, 2),
