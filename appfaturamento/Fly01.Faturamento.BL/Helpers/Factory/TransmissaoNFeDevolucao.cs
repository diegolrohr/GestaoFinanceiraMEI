...
namespace Fly01.Faturamento.BL.Helpers.Factory
{
    public class TransmissaoNFeDevolucao : TransmissaoNFe
    {
        ...
        public override TransmissaoVM ObterTransmissaoVM()
        {
            ...
            itemTransmissao.Identificador.NFReferenciada = ObterNFReferenciada();
        ...
        private void CalculaSTICMSPai(NFeProduto item, NotaFiscalItemTributacao itemTributacao, ICMSPai ICMS)
        {
            if (itemTributacao.CalculaST)
            {
                ...
                var st = TransmissaoBLs.SubstituicaoTributariaBL.AllIncluding(y => y.EstadoOrigem, y => y.EstadoDestino).AsNoTracking().Where(x =>
                    ...
                    x.EstadoDestino.Sigla == Cabecalho.Cliente.Estado.Sigla &&
                    x.TipoSubstituicaoTributaria == TipoSubstituicaoTributaria.Entrada
                    ).FirstOrDefault();
            ...
            if (itemTributacao.CalculaIPI)
            {
                return new IPIPai()
                {
                    CodigoST = item.GrupoTributario.TipoTributacaoIPI.HasValue ?
                        (CSTIPI)Enum.Parse(typeof(CSTIPI), item.GrupoTributario.TipoTributacaoIPI.Value.ToString()) : CSTIPI.OutrasEntradas,
