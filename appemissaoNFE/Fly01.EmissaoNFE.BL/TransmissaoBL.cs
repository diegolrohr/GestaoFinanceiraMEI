            ...
            foreach (var item in entity.Item)
            {
                ...
                if (item.Transporte.Volume != null)
                {
                    if (item.Transporte.ModalidadeFrete != ModalidadeFrete.SemFrete)
                    {
                        ...
                        entity.Fail(string.IsNullOrEmpty(item.Transporte.Volume.Marca) || (item.Transporte.Volume.Marca?.Length > 60),
                            new Error("Marca do volume é um dado obrigatório de no máximo 60 caracteres.", "Item.Transporte.Volume.marca"));
                        entity.Fail(string.IsNullOrEmpty(item.Transporte.Volume.Numeracao) || (item.Transporte.Volume.Numeracao?.Length > 60),
                            new Error("Numeração do volume é um dado obrigatório de no máximo 60 caracteres.", "Item.Transporte.Volume.Numeracao"));
