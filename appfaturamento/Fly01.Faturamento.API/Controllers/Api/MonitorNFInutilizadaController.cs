...
{
    [RoutePrefix("monitorNFInutilizada")]
    public class MonitorNFInutilizadaController : ApiBaseController
    {
        private async Task ProcessaStatusTSS()
        {
            using (UnitOfWork unitOfWork = new UnitOfWork(ContextInitialize))
            {
                unitOfWork.MonitorNFBL.AtualizaStatusTSSInutilizada(string.Empty);
